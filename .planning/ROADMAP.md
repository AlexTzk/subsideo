...
**Requirements**: CSLC-07, CSLC-08, CSLC-09, CSLC-12, RTCSUP-01, RTCSUP-03
...
**Success Criteria** (what must be TRUE):
  ...
  5. User runs DISP and CSLC EGMS L2a comparisons end-to-end without hitting the three v1.1 audit-flagged shared-infra defects: CR-01 (rasterio nodata access after context close), CR-02 (silently-swallowed HTTPError on retry), HI-01 (reproject() default-zero nodata); regression tests pin each fix (RTCSUP-03).
  6. User runs the v1.1 stale tests `test_env07_diff_discipline` and `test_iberian_aoi_fallback_chain_two_entries` and either both pass on the post-AOI-regeneration codebase or are explicitly removed with a documented rationale (CSLC-12).
...
**Success Criteria** (what must be TRUE):
  ...
  5. User opens `.planning/REQUIREMENTS.md` after v1.2 closure and the traceability table shows zero stale "Pending" rows for v1.1 requirements that VERIFICATION.md marked SATISFIED — refreshed against the v1.1-MILESTONE-AUDIT.md cross-reference (VAL-04).
