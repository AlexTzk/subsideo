...
`subsideo` is a Python library that produces OPERA-equivalent geospatial products (RTC, CSLC, DISP, DSWx, DIST) for EU areas of interest using Sentinel-1/2 data from the Copernicus Data Space Ecosystem (CDSE). It wraps the ISCE3/dolphin algorithm stack with a clean API and includes a validation framework comparing against OPERA N.Am. and EGMS EU reference products.

## Build & Install

Two-layer install required — ISCE3, GDAL, dolphin, tophu, snaphu are **conda-forge only**:

```bash
conda env create -f conda-env.yml    # heavy native deps (isce3, gdal, snaphu, dolphin, tophu)
conda activate subsideo
pip install -e ".[dev]"              # pure-Python package + dev tools
```

**Never** `pip install isce3` or `pip install dolphin` — they must come from conda-forge. The `dolphin` on PyPI is an unrelated GPU ray tracer.

## Rules
- Always use Context7 when I need library/API documentation, code generation, setup or configuration steps without me having to explicitly ask.
- Always use micromamba environment subsideo for running python commands, that's where all the packages will be present.
...
# Validation tests (full pipeline, run before release)
pytest -m validation
...
pytest is configured with `--cov=subsideo --cov-report=term-missing` and 80% coverage minimum.

## Architecture

```
src/subsideo/
├── cli.py              # typer CLI: subsideo rtc|cslc|disp|dswx|validate
├── config.py           # Pydantic BaseSettings (env vars + .env fallback)
├── data/               # All I/O: CDSE, ASF DAAC, DEM, orbits, ionosphere
├── burst/              # EU burst database (extends opera-utils for EU coverage)
├── products/           # One module per product: rtc, cslc, disp, dswx, dist
├── validation/         # Metrics + cross-comparison vs OPERA/EGMS/JRC
└── utils/              # I/O helpers, CRS/UTM utils, logging
```

**Data flow**: `data/` fetches inputs (SLC, DEM, orbits) → `burst/` resolves EU burst/frame geometry → `products/` runs processing pipelines → `validation/` compares outputs against reference products.

**Key boundary**: `data/cdse.py` uses CDSE S3 (`s3://eodata/`) with a custom endpoint (`endpoint_url=https://eodata.dataspace.copernicus.eu`), not standard AWS S3. ASF DAAC access (`data/asf.py`) is validation-only.

## Critical Constraints

- **EU burst DB**: OPERA's `opera-burstdb` only covers N.Am. `burst/db.py` must build an EU-scoped SQLite from ESA's published burst ID maps. Cache at `~/.subsideo/eu_burst_db.sqlite`.
- **UTM zones**: EU spans zones 28N–38N. Multi-zone AOIs and zone-boundary edge cases must be handled in `burst/tiling.py`.
- **Tropospheric correction**: MintPy uses ERA5 via CDS API — requires a separate `~/.cdsapirc` key.
- **SNAPHU**: Non-commercial research license. Available via conda-forge; `tophu` parallelises on top.
- **Credentials**: CDSE (`CDSE_CLIENT_ID`, `CDSE_CLIENT_SECRET`) and Earthdata (`EARTHDATA_USERNAME`, `EARTHDATA_PASSWORD`) via env vars / `.env`. Never commit credentials.
...
- `@pytest.mark.validation` — full pipeline cross-comparison runs
...
- **Validation pass criteria**: RTC RMSE < 0.5 dB, r > 0.99; CSLC amplitude r > 0.6, RMSE < 4 dB (phase-based criteria inapplicable across isce3 versions — see CONCLUSIONS_CSLC_N_AM.md §5); DISP r > 0.92, bias < 3 mm/yr; DSWx F1 > 0.90
<!-- GSD:project-end -->
